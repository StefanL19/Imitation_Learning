...
class NMTDecoder(nn.Module):
    def __init__(self, num_unigram_embeddings, num_bigram_embeddings, embedding_size, rnn_hidden_size, bos_index, attention_mechanism, training_mode=False):
        """
        Args:
        ...
        self.target_unigram_embedding = nn.Embedding(num_embeddings=num_unigram_embeddings, 
        ...
        self.num_unigram_embeddings = num_unigram_embeddings
        self.num_bigram_embeddings = num_bigram_embeddings
    ...
    def forward(self, encoder_state, initial_hidden_state, target_sequence, sample_probability=0.0):
        ...
        for i in range(output_sequence_size):
            ...
            y_input_vector = self.target_unigram_embedding(y_t_index)
            ...
            bigram_dot_similarity = self.simplex_projection(bigram_dot_similarity)

            if use_sample:
                #p_y_t_index = F.softmax(score_for_y_t_index * self._sampling_temperature, dim=1)
                p_y_t_index = F.softmax(dot_similarity, dim=1) #self.simplex_projection(dot_similarity) #F.softmax(dot_similarity, dim=1)
            ...
            bigram_output_vectors.append(bigram_dot_similarity)

        output_vectors = torch.stack(output_vectors).permute(1, 0, 2)
        bigram_output_vectors = torch.stack(bigram_output_vectors).permute(1, 0, 2)
        ...
        return output_vectors, bigram_output_vectors, stacked_attentions
